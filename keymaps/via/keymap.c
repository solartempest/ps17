...
void keyboard_post_init_user(void) {	//run as last task in keyboard init
	...
	//rgblight_mode_noeeprom(RGBLIGHT_MODE_KNIGHT); //For LED mapping sequence testing or demo
	rgblight_set();
  #endif
};
...
layer_state_t layer_state_set_user(layer_state_t state) {
	switch (get_highest_layer(state)) {
		...
		static uint16_t layer0_hueincrement = 3; //hue gradient colour increment
		...
		static uint16_t layer2_huestart = 135; //hue gradient starting colour - blue/purple
		static uint16_t layer2_hueincrement = 3; //hue gradient colour increment
		static uint16_t layer3_huestart = 220; //hue gradient starting colour - magenta/orange
